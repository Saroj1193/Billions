//
//  PopupOverGridMenuController.swift
//  BillionsApp
//
...
class PopupOverGridMenuController: UIViewController {

    private static let cell = "PopupOverGridMenuCell"
    
    private var collectionView: UICollectionView!
    private var backgroundView: UIView!
    public var isPresented = false
    public var isPopover = false
    public var appear = PopupOverGridMenu.PopupOverGridMenuAppear.fromTop
    public var dismiss = PopupOverGridMenu.PopupOverGridMenuDismiss.toBottom
    public var items = [PopupMenuItem]()
    public var itemSize = CGSize(width: 0, height: 0)
    public var action: ((PopupMenuItem) -> Void)?
    public var completion: (() -> Void)?
    
    private lazy var transitionController = PopupOverMenuTransitionController()
    ...
    override func viewDidLoad() {
        ...
        self.backgroundView.backgroundColor = UIColor.bgColor
        ...
        self.collectionView.register(UINib(nibName: "PopupOverGridMenuCell", bundle: nil), forCellWithReuseIdentifier: PopupOverGridMenuController.cell)
...
extension PopupOverGridMenuController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: PopupOverGridMenuController.cell, for: indexPath) as! PopupOverGridMenuCell
        
        let item = self.items[indexPath.row]
        if !item.text.isEmpty {
            cell.lblText.text = item.text
        } else if item.attributedText.length > 0 {
            cell.lblText.attributedText = item.attributedText
        }
...
extension PopupOverGridMenuController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let cell = collectionView.cellForItem(at: indexPath) as!  PopupOverGridMenuCell
        let item = self.items[indexPath.row]
...
extension PopupOverGridMenuController: UICollectionViewDelegateFlowLayout {
...
extension PopupOverGridMenuController: UIViewControllerTransitioningDelegate {
...
extension PopupOverGridMenuController: UIPopoverPresentationControllerDelegate {
